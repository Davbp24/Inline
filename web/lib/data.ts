...
// ---------------------------------------------------------------------------
// Agent time saved (multi-agent system)
// ---------------------------------------------------------------------------
...
export async function fetchAgentMinutesSaved(workspaceId?: string): Promise<number> {
  ...
  try {
    ...
    if (error || !runs) return 0

    let minutesSaved = 0
    for (const r of runs as { intent: string }[]) {
      minutesSaved += MINUTES_SAVED_BY_INTENT[r.intent] ?? 2
    }
    /* eslint-enable @typescript-eslint/no-explicit-any */
