...
import { InlineChatBadge, InlineChatIcon } from './InlineChatIcon'
import { PANEL as C, TOOL_TILE } from '../lib/extensionTheme'
...
/** Header mark for tool panel popups — navy tile matching the web chat badge. */
export function ToolHeaderIcon({ tool, size = 24 }: { tool: ToolId; size?: number }) {
  if (tool === 'ai') {
    return <InlineChatBadge size={size} iconSize={Math.round(size * 0.54)} />
  }
  return (
    <ToolIconTile size={size}>
      {renderToolIcon(tool, Math.round(size * 0.72))}
    </ToolIconTile>
  )
}
